...
void test('rejects an unconfigured or simulated provider', () => {
...
void test('configures OpenRouter with its Responses endpoint and attribution headers', () => {
  ...
  assert.equal(config.extraHeaders['X-Title'], 'Prompt Lab');
});

void test('keeps direct OpenAI configuration separate from OpenRouter', () => {
  const config = resolveAgentProvider({
    AI_PROVIDER: 'openai',
